  ...
  class CustomerSearcher
    ...
    def paginate
      if valid? 
        ...
        Customer.none.paginate(per_page: PER_PAGE, page: page)
      end
    end
    ...
    def find
      reln = approval_status == 'U' ? Customer.unscoped.where("approval_status =?",'U').order("id desc") : Customer.order("id desc")
